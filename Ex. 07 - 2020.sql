...
/* exibir o nome dos 2 primeiros alunos cadastrados. */          
select * from tbaluno limit 2;

/* Exibir os 2 últimos cursos cadastrados. */
select nomeCurso as "Últimos" from tbcurso order by codCurso desc limit 2;

/* Exibir o nome e o valor do curso mais caro. */
select nomeCurso as "Curso mais caro", max(valorCurso) as "Valor" from tbcurso;

/* Exibir todos os dados da ultima matrícula. */
select * from tbmatricula order by codMatricula desc limit 1;

/* Exibir os dados de 2 alunos de SP. */
select * from tbaluno where naturalidadeAluno = 'SP' order by "Dois Alunos de SP" limit 2;
